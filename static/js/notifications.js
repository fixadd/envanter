(() => {
  ...
  const alertContentEl = alertModalEl.querySelector(".modal-content");
  const alertHeaderEl = alertModalEl.querySelector(".modal-header");
  const alertFooterEl = alertModalEl.querySelector(".modal-footer");
  const alertCloseBtn = alertHeaderEl?.querySelector?.(".btn-close") || null;
  ...
  const VARIANT_CLASSNAMES = Object.keys(VARIANT_DEFAULTS).map(
    (variant) => `modal-variant-${variant}`,
  );
  ...
  const LIGHT_CLOSE_VARIANTS = new Set(["warning", "light"]);
  ...
  function applyAlertVariant(variant) {
    ...
    if (alertContentEl) {
      alertContentEl.dataset.variant = finalVariant;
    }
    if (alertHeaderEl) {
      alertHeaderEl.dataset.variant = finalVariant;
    }
    if (alertFooterEl) {
      alertFooterEl.dataset.variant = finalVariant;
    }
    if (alertCloseBtn) {
      if (LIGHT_CLOSE_VARIANTS.has(finalVariant)) {
        alertCloseBtn.classList.remove("btn-close-white");
      } else {
        alertCloseBtn.classList.add("btn-close-white");
      }
    }
  }
