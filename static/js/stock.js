  ...
  try {
    ...
    sel.innerHTML =
      ...
      data
        .map(
          (x) =>
            `<option value="${x.text || x.name || ""}" data-id="${x.id || ""}">${x.text || x.name || ""}</option>`,
        )
        .join("");
  } catch (e) {
...
document
  .getElementById("stockAddModal")
  ?.addEventListener("shown.bs.modal", initLookups);
...
async function loadModels() {
  ...
  if (!markaId) return;
  try {
    const res = await fetch(`/api/picker/model?marka_id=${markaId}`, {
...
async function sa_loadStocks() {
  ...
  try {
    const res = await fetch(URL_STOCK_OPTIONS, {
      headers: { Accept: "application/json" },
    });
    if (!res.ok) {
      showBanner(`Stok listesi başarısız (HTTP ${res.status})`);
      return;
    }
    ...
    data.forEach((o) => {
      ...
      opt.textContent =
        o.label ??
        `${o.donanim_tipi || "Donanım"} | IFS:${o.ifs_no || "-"} | Mevcut:${o.mevcut_miktar ?? 0}`;
    ...
    showBanner("Stok listesi yüklenemedi. Konsolu kontrol edin.");
    console.error(e);
  }
}
...
async function sa_submit() {
  ...
  if (!stockId) {
    showBanner("Lütfen stok seçiniz.", "warning");
    return;
  }
...
stockStatusModal?.addEventListener("show.bs.modal", () => {
  fetch("/api/stock/status")
    ...
    .then((d) => {
      ...
      tbody.innerHTML = Object.entries(d.totals || {})
        .map(([dt, qty]) => {
          const det = detail[dt];
          const id = "det" + dt.replace(/[^a-zA-Z0-9]/g, "");
          const btn = det
            ? `<button class="btn btn-sm btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#${id}"><i class="bi bi-list"></i></button>`
            : "";
          const detailRows = det
            ? `<tr id="${id}" class="collapse"><td colspan="3"><table class="table table-sm mb-0">${Object.entries(
                det,
              )
                .map(([ifs, q]) => `<tr><td>${ifs}</td><td>${q}</td></tr>`)
                .join("")}</table></td></tr>`
            : "";
