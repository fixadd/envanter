...
fetch("/api/lookup/donanim_tipi")
  ...
  .then((d) => {
    ...
    sel.innerHTML =
      ...
      (d.items || []).map((x) => `<option>${x.ad}</option>`).join("");
  });
...
["ttLisans", "ttEnvanter", "ttYazici"].forEach((id) => {
  document
    .getElementById(id)
    ?.addEventListener("change", (e) => showSection(e.target.value));
});
  ...
  if (invSel)
    invSel.innerHTML =
      ...
      (src.inventories || [])
        .map(
          (i) =>
            `<option value="${i.id}">${i.envanter_no} - ${i.marka || ""} ${i.model || ""}</option>`,
        )
        .join("");
  if (licSel)
    licSel.innerHTML =
      ...
      (lic.items || [])
        .map(
          (l) =>
            `<option value="${l.id}">${l.lisans_adi} ${l.lisans_anahtari ? "(" + l.lisans_anahtari + ")" : ""}</option>`,
        )
        .join("");
  if (prnSel)
    prnSel.innerHTML =
      ...
      (prn.items || [])
        .map(
          (p) =>
            `<option value="${p.id}">${p.marka || ""} ${p.model || ""} ${p.seri_no ? "(" + p.seri_no + ")" : ""}</option>`,
        )
        .join("");
});
